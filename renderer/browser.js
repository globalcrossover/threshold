...
function toggleDayMode() {
  ...
  localStorage.setItem(DAY_MODE_KEY, isDay ? '1' : '0')
  if ($dayModeBtn) {
    $dayModeBtn.innerHTML = isDay ? '&#9790;' : '&#9788;'
    $dayModeBtn.title     = isDay ? 'Switch to night mode' : 'Switch to day mode'
  }
}

if ($dayModeBtn) $dayModeBtn.addEventListener('click', toggleDayMode)
...
function loadBookmarks() {
  try {
    ...
    if (oldRaw) { bookmarks = migrateOldBookmarks(JSON.parse(oldRaw)); saveBookmarks(); return }
  } catch(e) {}
...
function renderBookmarksView(homePage) {
  ...
  var addLinkBtn = document.createElement('button'); addLinkBtn.className = 'bm-tool-btn'; addLinkBtn.innerHTML = '&#43;&nbsp; Add Link'
  addLinkBtn.addEventListener('click', function() { openAddLinkModal(null) })
  var importBtn = document.createElement('button'); importBtn.className = 'bm-tool-btn bm-import-btn'; importBtn.innerHTML = '&#8679;&nbsp; Import from Browser'
...
function renderFolder(folder, homePage) {
  var el = document.createElement('div'); el.className = 'bm-folder' + (folder.open ? ' open' : '')
    ...
    (function(card) {
      ...
      ;(function(c) {
        ...
        del.addEventListener('click', function(e) {
          ...
          saveUserCards(); renderAllUserCardViews()
        })
      })(card)
...
function openUserCardModal() {
  ...
  bg.innerHTML =
    '<div id="bm-modal"><h3>Add Card</h3>' +
    ...
    '<button class="modal-btn primary" id="bm-save">Add</button></div></div>'
...
function openVaultAddModal() {
  ...
  var bg = document.createElement('div'); bg.id = 'bm-modal-bg'; bg.className = 'open'
  bg.innerHTML =
    '<div id="bm-modal"><h3>\uD83D\uDD10 Save Credential</h3>' +
    ...
    '<div id="bm-modal-btns"><button class="modal-btn" id="bm-cancel">Cancel</button>' +
    '<button class="modal-btn primary" id="bm-save">Save to Vault</button></div></div>'
  $contentArea.appendChild(bg)
  bg.addEventListener('click', function(e) { if (e.target === bg) closeBmModal() })
  document.getElementById('bm-cancel').addEventListener('click', closeBmModal)
  document.getElementById('bm-save').addEventListener('click', function() {
...
function renderSearchResults(container, query, results) {
  var inner = '<div class="search-page-inner"><div class="search-header"><div class="search-logo">THRESHOLD</div><div class="search-input-wrap"><input class="search-page-input" type="text" value="' + escHtml(query) + '" /></div></div>'
  if (results.length === 0) {
...
if (window.threshold && window.threshold.onMessage) {
  ...
  window.threshold.onMessage('ctx-search',   function(query) { var tab=getTab(activeId); if(tab) navigateTab(tab.id, SEARCH_PREFIX+encodeURIComponent(query)) })
  window.threshold.onMessage('ctx-inspect',  function(x, y) { var tab=getTab(activeId); if(tab&&tab.webview) tab.webview.inspectElement(x, y) })
}

if (window.threshold && window.threshold.platform) document.body.classList.add(window.threshold.platform)
